...
def print_version():
    ...
    c_ext_status = (
        "Enabled (fast_matcher)" if C_EXTENSION_AVAILABLE else "Disabled (pure-Python fallback)"
    )
...
def _print_title_banner():
    """Prints the application title banner for the CLI."""
    ...
    banner_panel = Panel(
        ...
        title="[bold cyan]Error Translator[/bold cyan]",
        subtitle="[dim white]Offline Python Traceback Diagnostics[/dim white]",
        padding=(1, 4),
    )
...
def print_about():
    """Prints a structured 'about' view showcasing project metadata, runtime features, and examples."""
    _print_title_banner()
    ...
    features = Text()
    features.append("• Offline deterministic rule matching\n", style="white")
    features.append("• Source context and traceback location parsing\n", style="white")
    features.append("• Actionable remediation suggestions\n", style="white")
    features.append("• AST-level lexical scope analysis", style="white")
    console.print(
        Panel(features, title="[bold green]Features[/]", border_style="green", expand=False)
    )
    ...
    console.print(
        Panel(examples, title="[bold yellow]Quick Start[/]", border_style="yellow", expand=False)
    )
...
def print_help():
    ...
    cli_table.add_row(
        "explain-error run <script_path>",
        "Execute a Python script, intercept, and translate tracebacks.",
    )
    cli_table.add_row(
        'explain-error "<traceback_text>"',
        "Translate a raw traceback string passed as an argument.",
    )
    cli_table.add_row(
    ...
    console.print(
        Panel(
            cli_table,
            title="[bold cyan]1. Command Line Interface (CLI) Usage[/]",
            border_style="cyan",
            expand=False,
        )
    )
    ...
    python_info.append(
        "Import this module at the top of your script to automatically capture and translate all unhandled exceptions:\n",
        style="dim white",
    )
    python_info.append("    import error_translator.auto\n\n", style="bold white")
    python_info.append("B) Programmatic Translation API\n", style="bold magenta")
    python_info.append(
        "Translate exception string programmatically within your codebase:\n", style="dim white"
    )
    ...
    console.print(
        Panel(
            python_info,
            title="[bold magenta]2. Python Library Integration[/]",
    ...
    fastapi_info = Text()
    fastapi_info.append(
        "Run the local HTTP REST API and interactive Web UI dashboard:\n", style="dim white"
    )
    fastapi_info.append(
        "    uvicorn error_translator.api.server:app --host 127.0.0.1 --port 8000\n\n",
        style="bold yellow",
    )
    fastapi_info.append("Exposes:\n", style="dim white")
    fastapi_info.append("  • GET  /        -> Interactive Web UI dashboard\n", style="white")
    fastapi_info.append(
        "  • POST /translate -> REST endpoint for remote traceback translation", style="white"
    )

    console.print(
        Panel(
            fastapi_info,
            title="[bold yellow]3. Local Web UI & API Service[/]",
            border_style="yellow",
            expand=False,
        )
    )
    ...
    console.print(
        Panel(
            options_table,
            title="[bold green]4. CLI Options & Flags[/]",
            border_style="green",
            expand=False,
        )
    )
    ...
    console.print(
        Panel(
            links_table,
            title="[bold blue]5. Documentation & Resources[/]",
            border_style="blue",
            expand=False,
        )
    )
...
def print_result(result: dict):
    ...
    if file_name and file_name != "Unknown File":
        console.print(
            Panel(
                f"[bold yellow]File:[/] [bold white]{file_name}\n[bold yellow]Line:[/] [bold white]{line_no}",
    ...
    if result.get("code"):
        ...
        console.print(
            Panel(syntax, title="[bold blue]Code Context[/]", border_style="blue", box=box.ROUNDED)
        )

    explanation = result.get("explanation", "No explanation available.")
    console.print(
        Panel(
            f"[bold white]{explanation}[/]",
    ...
    if result.get("ast_insight"):
        console.print(
            Panel(
                f"[bold white]{result['ast_insight']}[/]",
...
def print_execution_error(
    script_name: str, message: str, as_json: bool, error_type: str = "Execution Error"
):
